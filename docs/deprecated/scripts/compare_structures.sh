...
for table in "${TABLES[@]}"; do
  ...
  mysql -h 10.3.172.11 -u erdms_user -p'CHANGE_ME_DB_PASSWORD' eeo2025-dev -e "SHOW CREATE TABLE \`$table\`" 2>&1 | tail -n +2 > /tmp/dev_$table.txt
  
  # Get structure from PROD  
  mysql -h 10.3.172.11 -u erdms_user -p'CHANGE_ME_DB_PASSWORD' eeo2025 -e "SHOW CREATE TABLE \`$table\`" 2>&1 | tail -n +2 > /tmp/prod_$table.txt
