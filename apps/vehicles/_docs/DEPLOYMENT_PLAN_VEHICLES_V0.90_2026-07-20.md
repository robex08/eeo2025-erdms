# Deployment Plan - Vehicles v0.90
...
1. Nasadit frontend/backend zmeny Vehicles v0.90.
...
Migrace pro v0.90 + vecerni rozsirení:
...
```bash
BACKUP_ROOT="/var/www/__BCK_PRODUKCE/vehicles_v0_90_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BACKUP_ROOT"
...
```bash
mysqldump -h <DB_HOST> -u <DB_USER> -p <DB_NAME> | gzip > "$BACKUP_ROOT/db_before_v0_90.sql.gz"
```
