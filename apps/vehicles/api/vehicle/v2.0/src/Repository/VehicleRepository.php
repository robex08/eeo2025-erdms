...
final class VehicleRepository
{
    ...
    private const TBL_CARD_AUDIT = 'vehicles_card_audit_v2';
    private const TBL_IDENTITY_ALIASES = 'vehicles_identity_aliases_v2';
    private const TBL_CARD_ATTACHMENTS = 'vehicles_card_attachments_v2';
    private const TBL_SERVICE_RECORDS = 'vehicles_service_records_v2';
    private const TBL_VEHICLE_EQUIPMENT = 'vehicles_vehicle_equipment_v2';
    private const TBL_INSURANCE_POLICIES = 'vehicles_insurance_policies_v2';
    ...
    ): array
    {
        ...
        $fromSql .= '
            LEFT JOIN (
                SELECT vehicle_id, "A" AS funding_flag
                FROM vehicles_vehicle_funding_v2
        ...
        ";
            ...
            $datumZarazeniSelect = $hasInServiceFrom
                ? 'DATE_FORMAT(d.in_service_from, "%Y-%m-%d %H:%i:%s") AS datum_zarazeni'
        ...
        $dotaceSelect = 'COALESCE(funding.funding_flag, "") AS dotace';
        ...
        END';
        $yearLabelSql = $hasInServiceFrom
            ? 'CASE
                WHEN d.in_service_from IS NULL THEN "Nezadáno"
                ELSE CAST(YEAR(d.in_service_from) AS CHAR)
            END'
            : '"Nezadáno"';
        ...
        $dotaceCountSql = ', COUNT(DISTINCT CASE WHEN funding.funding_flag = "A" THEN v.id END) AS dotace_count';
    ...
    public function saveLookupItem(array $payload): array
    {
        ...
        $select->execute(['category' => $payload['category'], 'code' => $payload['code']]);
        return $select->fetch() ?: $payload;
    }
    ...
    public function upsertCarsFromWebDispecink(array $cars): int
    {
        ...
        foreach ($cars as $car) {
            ...
            $incomingSpz = trim((string) ($car['identifier'] ?? ''));
            $incomingSpzNormalized = $this->normalizeVehicleIdentity($incomingSpz);
            $existingStmt = $this->pdo->prepare(
                'SELECT id, legacy_carid, spz
                 FROM vehicles_cars_list_v2
                 WHERE legacy_carid = :legacy_carid
                 LIMIT 1'
            );
            $existingStmt->execute(['legacy_carid' => $carId]);
            $existing = $existingStmt->fetch() ?: null;

            if ($incomingSpzNormalized !== '') {
                $conflictStmt = $this->pdo->prepare(
                    'SELECT id, legacy_carid, spz
                     FROM vehicles_cars_list_v2
                     WHERE UPPER(REPLACE(TRIM(spz), " ", "")) = :spz_normalized
                     LIMIT 1'
                );
                $conflictStmt->execute(['spz_normalized' => $incomingSpzNormalized]);
                $conflict = $conflictStmt->fetch() ?: null;
                if ($conflict !== null && (int) $conflict['id'] !== (int) ($existing['id'] ?? 0)) {
                    throw new RuntimeException(sprintf(
                        'WebDispecink SPZ %s koliduje s vozidlem %d (legacy_carid %d). Synchronizace zastavena.',
                        $incomingSpz,
                        (int) $conflict['id'],
                        (int) $conflict['legacy_carid']
                    ));
                }
            }
            ...
            if ($this->tableExists(self::TBL_IDENTITY_ALIASES)) {
                $vehicleId = (int) ($existing['id'] ?? 0);
                if ($vehicleId <= 0) {
                    $vehicleStmt = $this->pdo->prepare(
                        'SELECT id FROM vehicles_cars_list_v2 WHERE legacy_carid = :legacy_carid LIMIT 1'
                    );
                    $vehicleStmt->execute(['legacy_carid' => $carId]);
                    $vehicleId = (int) ($vehicleStmt->fetchColumn() ?: 0);
                }

                if ($vehicleId > 0 && $incomingSpzNormalized !== '') {
                    $currentAliasStmt = $this->pdo->prepare(
                        'SELECT identity_value, identity_value_normalized
                         FROM ' . self::TBL_IDENTITY_ALIASES . '
                         WHERE vehicle_id = :vehicle_id
                           AND identity_type = "spz"
                           AND is_current = 1
                         ORDER BY id DESC
                         LIMIT 1'
                    );
                    $currentAliasStmt->execute(['vehicle_id' => $vehicleId]);
                    $currentAlias = $currentAliasStmt->fetch() ?: null;
                    $oldSpz = $currentAlias['identity_value'] ?? ($existing['spz'] ?? null);
                    $oldSpzNormalized = $currentAlias['identity_value_normalized'] ?? $this->normalizeVehicleIdentity((string) ($existing['spz'] ?? ''));

                    if ($oldSpzNormalized !== $incomingSpzNormalized) {
                        $closeAliasStmt = $this->pdo->prepare(
                            'UPDATE ' . self::TBL_IDENTITY_ALIASES . '
                             SET valid_to = :valid_to, is_current = 0
                             WHERE vehicle_id = :vehicle_id
                               AND identity_type = "spz"
                               AND is_current = 1'
                        );
                        $closeAliasStmt->execute([
                            'valid_to' => $now,
                            'vehicle_id' => $vehicleId,
                        ]);

                        $newAliasStmt = $this->pdo->prepare(
                            'INSERT INTO ' . self::TBL_IDENTITY_ALIASES . ' (
                                vehicle_id, identity_type, identity_value, identity_value_normalized,
                                valid_from, is_current, source, created_at
                            ) VALUES (
                                :vehicle_id, "spz", :identity_value, :identity_value_normalized,
                                :valid_from, 1, "webdispecink", :created_at
                            )'
                        );
                        $newAliasStmt->execute([
                            'vehicle_id' => $vehicleId,
                            'identity_value' => $incomingSpz,
                            'identity_value_normalized' => $incomingSpzNormalized,
                            'valid_from' => $now,
                            'created_at' => $now,
                        ]);

                        if ($this->tableExists(self::TBL_CARD_AUDIT)) {
                            $auditStmt = $this->pdo->prepare(
                                'INSERT INTO ' . self::TBL_CARD_AUDIT . ' (
                                    vehicle_id, event_type, field_name, old_value, new_value,
                                    old_spz, new_spz, actor_type, source, occurred_at
                                ) VALUES (
                                    :vehicle_id, "spz_changed", "spz", :old_value, :new_value,
                                    :old_spz, :new_spz, "webdispecink", "webdispecink_sync", :occurred_at
                                )'
                            );
                            $auditStmt->execute([
                                'vehicle_id' => $vehicleId,
                                'old_value' => $oldSpz,
                                'new_value' => $incomingSpz,
                                'old_spz' => $oldSpz,
                                'new_spz' => $incomingSpz,
                                'occurred_at' => $now,
                            ]);
    ...
    private function normalizeVehicleIdentity(string $value): string
    {
    ...
    public function upsertGeneralInfoFromWebDispecink(array $rows): int
    {
        ...
        $upsertStmt = $this->pdo->prepare(
            'INSERT INTO vehicles_detail_cards (vehicle_id, w_popis, w_stanoviste, w_nadrz, updated_at)
             VALUES (:vehicle_id, :w_popis, :w_stanoviste, :w_nadrz, CURRENT_TIMESTAMP)
             ON DUPLICATE KEY UPDATE
        ...
        foreach ($rows as $row) {
            ...
            $wPopis = trim((string) ($row['w_popis'] ?? ''));
            $upsertStmt->execute([
                'vehicle_id' => $vehicleId,
                'w_popis' => $wPopis === '' ? null : $wPopis,
                'w_stanoviste' => $row['w_stanoviste'] ?? null,
                'w_nadrz' => $row['w_nadrz'] ?? null,
            ]);
    ...
    public function getDashboardMetrics(string $statusFilter = 'all', int $actorUserId = 0, bool $actorHasAllVehicles = true): array
    {
        ...
        $dotaceStmt = $this->pdo->prepare(
            'SELECT COUNT(DISTINCT CASE WHEN f.funding_status_code IN ("awarded", "sustainability") THEN v.id END) AS dotace_count
             FROM vehicles_cars_list_v2 v
             LEFT JOIN vehicles_vehicle_funding_v2 f ON f.vehicle_id = v.id AND f.deleted_at IS NULL
             ' . $accessJoin . '
             ' . $aliasWhere
        );
    ...
    public function getVehicleDetailById(int $vehicleId, int $actorUserId = 0, bool $actorHasAllVehicles = true): ?array
    {
        ...
        $hasPositionsKm = $this->tableExists(self::TBL_WD_POSITIONS)
            && $this->columnExists(self::TBL_WD_POSITIONS, 'w_carid')
            && $this->columnExists(self::TBL_WD_POSITIONS, 'w_km');
        // Do not fall back to YEAR(in_service_from) here: acquisition_year is a distinct manual field and
        // merging in the WD-derived "datum zařazení" year made the form silently resubmit/audit it as a real edit.
        $acquisitionYearSelect = 'NULLIF(d.acquisition_year, 0) AS acquisition_year';
        $datumZarazeniSelect = $hasInServiceFrom
            ? 'DATE_FORMAT(d.in_service_from, "%Y-%m-%d %H:%i:%s") AS datum_zarazeni'
            : 'NULL AS datum_zarazeni';
        $najetoKmSelect = $hasPositionsKm
        ...
        $sql = 'SELECT
            ...
            v.legacy_carid,
                ...
                d.stk_valid_to,
                d.emission_valid_to,
                d.evidencni_cislo_zzs,
                d.vin,
                ' . $acquisitionYearSelect . ',
                d.acquisition_supplier,
                d.warranty_valid_to,
                d.acquisition_price,
                d.technical_condition_code,
                d.service_interval_km,
                d.service_interval_months,
                d.battery_condition_code,
                d.vehicle_lifetime_percent,
                 ' . $serviceContextJsonSelect . ',
                     ' . $manualLocationStateSelect . ',
                     ' . $manualLocationUpdatedAtSelect . ',
    ...
    public function saveVehicleDetailById(int $vehicleId, array $payload, int $actorUserId = 0): void
    {
        ...
        $hasManualLocationUpdatedAt = $this->columnExists('vehicles_detail_cards', 'manual_location_updated_at');
        $hasServiceContextJson = $this->columnExists('vehicles_detail_cards', 'service_context_json');
        $auditEnabled = $this->tableExists(self::TBL_CARD_AUDIT);
        $before = $auditEnabled ? $this->getVehicleDetailAuditSnapshot($vehicleId) : null;
        ...
        $updateAssignments = [
            ...
            'evidencni_cislo_zzs = VALUES(evidencni_cislo_zzs)',
            'vin = VALUES(vin)',
            'acquisition_year = VALUES(acquisition_year)',
            'acquisition_supplier = VALUES(acquisition_supplier)',
            'warranty_valid_to = VALUES(warranty_valid_to)',
            'acquisition_price = VALUES(acquisition_price)',
            'technical_condition_code = VALUES(technical_condition_code)',
            'service_interval_km = VALUES(service_interval_km)',
            'service_interval_months = VALUES(service_interval_months)',
            'battery_condition_code = VALUES(battery_condition_code)',
            'vehicle_lifetime_percent = VALUES(vehicle_lifetime_percent)',
        ];
        ...
        $params = [
            ...
            'evidencni_cislo_zzs' => $payload['evidencni_cislo_zzs'],
            'vin' => $payload['vin'],
            'acquisition_year' => $payload['acquisition_year'],
            'acquisition_supplier' => $payload['acquisition_supplier'],
            'warranty_valid_to' => $payload['warranty_valid_to'],
            'acquisition_price' => $payload['acquisition_price'],
            'technical_condition_code' => $payload['technical_condition_code'],
            'service_interval_km' => $payload['service_interval_km'],
            'service_interval_months' => $payload['service_interval_months'],
            'battery_condition_code' => $payload['battery_condition_code'],
            'vehicle_lifetime_percent' => $payload['vehicle_lifetime_percent'],
        ];
        ...
        try {
            ...
            if ($auditEnabled) {
                $after = $this->getVehicleDetailAuditSnapshot($vehicleId);
                $this->appendVehicleDetailAudit($vehicleId, $before, $after, $actorUserId);
            }
    ...
    private function getVehicleDetailAuditSnapshot(int $vehicleId): ?array
    {
        $columns = [
            'zzs_typ',
            'w_popis',
        ...
        $optionalColumns = [
            'manual_location_state',
            'manual_location_updated_at',
            'service_context_json',
        ];
        foreach ($optionalColumns as $column) {
            if ($this->columnExists('vehicles_detail_cards', $column)) {
                $columns[] = $column;
            }
        }
    ...
    private function appendVehicleDetailAudit(int $vehicleId, ?array $before, ?array $after, int $actorUserId): void
    {
        $fields = [
            'zzs_typ',
            'w_popis',
        ...
        foreach ($fields as $field) {
            ...
            // NULL and '' both mean "empty" for these fields, so treat them as unchanged to avoid bogus audit rows.
            $oldIsEmpty = $oldValue === null || $oldValue === '';
            $newIsEmpty = $newValue === null || $newValue === '';
            if ($oldIsEmpty && $newIsEmpty) {
                continue;
            }
            if (!$oldIsEmpty && !$newIsEmpty && (string) $oldValue === (string) $newValue) {
                continue;
            }
            ...
            $stmt = $this->pdo->prepare(
                'INSERT INTO ' . self::TBL_CARD_AUDIT . ' (
                    vehicle_id, event_type, field_name, old_value, new_value,
                    actor_user_id, actor_type, source, occurred_at
                ) VALUES (
                    :vehicle_id, :event_type, :field_name, :old_value, :new_value,
                    :actor_user_id, :actor_type, :source, :occurred_at
                )'
            ...
            $stmt->bindValue(':vehicle_id', $vehicleId, PDO::PARAM_INT);
            $stmt->bindValue(':event_type', 'field_changed', PDO::PARAM_STR);
            ...
            $stmt->bindValue(':actor_user_id', $actorUserId > 0 ? $actorUserId : null, $actorUserId > 0 ? PDO::PARAM_INT : PDO::PARAM_NULL);
            $stmt->bindValue(':actor_type', $actorUserId > 0 ? 'user' : 'system', PDO::PARAM_STR);
            $stmt->bindValue(':source', 'v2_detail', PDO::PARAM_STR);
            $stmt->bindValue(':occurred_at', $this->nowForDb(), PDO::PARAM_STR);
            $stmt->execute();
        }
    }
    ...
    public function listVehicleCardAudit(
        int $vehicleId,
        string $fieldName = '',
        int $limit = 100,
        int $actorUserId = 0,
        ...
        if (!$this->tableExists(self::TBL_CARD_AUDIT) || $vehicleId <= 0) {
            return [];
        }

        $limit = max(1, min(500, $limit));
        $restrictByAssignments = $actorUserId > 0 && !$actorHasAllVehicles;
        $hasUsersTable = $this->tableExists('vehicles_users');
        $actorNameSelect = $hasUsersTable
        ...
        $sql = 'SELECT
                    ...
                    a.sync_job_id,
                    a.correlation_id,
                    DATE_FORMAT(a.occurred_at, "%Y-%m-%d %H:%i:%s") AS occurred_at,
                    a.metadata_json
                FROM ' . self::TBL_CARD_AUDIT . ' a';

        if ($hasUsersTable) {
            $sql .= ' LEFT JOIN vehicles_users u ON u.id = a.actor_user_id';
        }

        $sql .= ' WHERE a.vehicle_id = :vehicle_id';

        $params = ['vehicle_id' => $vehicleId];
        if ($fieldName !== '') {
            $sql .= ' AND a.field_name = :field_name';
            $params['field_name'] = mb_substr(trim($fieldName), 0, 128);
        }
        ...
        $sql .= ' ORDER BY a.occurred_at DESC, a.id DESC LIMIT ' . $limit;
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($params);
    ...
    public function listVehicleAttachments(
        int $vehicleId,
        string $documentTypeCode = '',
        int $actorUserId = 0,
        bool $actorHasAllVehicles = true
    ): array {
        if (!$this->tableExists(self::TBL_CARD_ATTACHMENTS) || $vehicleId <= 0) {
            return [];
        }
        ...
        if ($documentTypeCode !== '') {
            $sql .= ' AND a.document_type_code = :document_type_code';
            $params['document_type_code'] = mb_substr(trim($documentTypeCode), 0, 64);
        }
    ...
    public function createVehicleAttachment(array $payload): int
    {
        $stmt = $this->pdo->prepare(
    ...
    public function findVehicleAttachmentById(
        ...
        if (!$this->tableExists(self::TBL_CARD_ATTACHMENTS) || $attachmentId <= 0) {
            return null;
        }
    ...
    public function softDeleteVehicleAttachment(int $attachmentId, int $actorUserId, bool $actorHasAllVehicles = true): ?array
    {
        $attachment = $this->findVehicleAttachmentById($attachmentId, $actorUserId, $actorHasAllVehicles);
        if ($attachment === null) {
            return null;
        }
    ...
    public function appendVehicleAuditEvent(array $payload): void
    {
        if (!$this->tableExists(self::TBL_CARD_AUDIT)) {
            return;
        }
    ...
    public function listVehicleServiceRecords(
        int $vehicleId,
        int $actorUserId = 0,
        bool $actorHasAllVehicles = true
    ): array {
        if (!$this->tableExists(self::TBL_SERVICE_RECORDS) || $vehicleId <= 0) {
            return [];
        }
        ...
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (
                        SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva
                        WHERE uva.vehicle_id = s.vehicle_id AND uva.user_id = :access_user_id
                    )';
            $params['access_user_id'] = $actorUserId;
        }
        $sql .= ' ORDER BY COALESCE(s.service_date, s.planned_date, DATE(s.created_at)) DESC, s.id DESC';
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($params);
        return $stmt->fetchAll() ?: [];
    }

    public function createVehicleServiceRecord(array $payload): int
    {
        $startedTransaction = false;
        if (!$this->pdo->inTransaction()) {
            $this->pdo->beginTransaction();
        ...
        try {
            ...
            if ($this->tableExists(self::TBL_MANUAL_EVENTS)) {
                $eventStmt = $this->pdo->prepare(
                    'INSERT INTO ' . self::TBL_MANUAL_EVENTS . ' (
                        vehicle_id, event_type, event_state, is_manual, service_name,
                        service_address, service_contact, note, metadata_json, source,
                        created_by_user_id, effective_at
                    ) VALUES (
                        :vehicle_id, "service_record", :event_state, 1, :service_name,
                        NULL, NULL, :note, :metadata_json, "v2_service", :created_by_user_id,
                        COALESCE(:service_date, CURRENT_TIMESTAMP)
                    )'
                );
                $eventStmt->execute([
                    'vehicle_id' => $payload['vehicle_id'],
                    ...
                    'metadata_json' => json_encode(['service_record_id' => $recordId], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
    ...
    public function listVehicleEquipment(int $vehicleId, int $actorUserId = 0, bool $actorHasAllVehicles = true): array
    {
        if (!$this->tableExists(self::TBL_VEHICLE_EQUIPMENT) || $vehicleId <= 0) {
            return [];
        }

        $sql = 'SELECT e.id, e.vehicle_id, e.equipment_type_code, e.status_code,
                       e.equipment_name, e.manufacturer, e.model, e.serial_number,
                       e.inventory_number, e.supplier_name, e.acquired_at,
                       e.warranty_valid_to, e.revision_valid_to, e.cost_amount,
                       e.cost_currency, e.note, e.source, e.created_by_user_id,
                       DATE_FORMAT(e.created_at, "%Y-%m-%d %H:%i:%s") AS created_at,
                       DATE_FORMAT(e.updated_at, "%Y-%m-%d %H:%i:%s") AS updated_at
                FROM ' . self::TBL_VEHICLE_EQUIPMENT . ' e
                WHERE e.vehicle_id = :vehicle_id AND e.deleted_at IS NULL';
        $params = ['vehicle_id' => $vehicleId];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (
                        SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva
                        WHERE uva.vehicle_id = e.vehicle_id AND uva.user_id = :access_user_id
                    )';
            $params['access_user_id'] = $actorUserId;
        }
        $sql .= ' ORDER BY e.equipment_type_code ASC, e.id DESC';
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute($params);
        return $stmt->fetchAll() ?: [];
    }

    public function createVehicleEquipment(array $payload): int
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO ' . self::TBL_VEHICLE_EQUIPMENT . ' (
                vehicle_id, equipment_type_code, status_code, equipment_name,
                manufacturer, model, serial_number, inventory_number, supplier_name,
                acquired_at, warranty_valid_to, revision_valid_to, cost_amount,
                cost_currency, note, source, created_by_user_id, updated_by_user_id,
                metadata_json
            ) VALUES (
                :vehicle_id, :equipment_type_code, :status_code, :equipment_name,
                :manufacturer, :model, :serial_number, :inventory_number, :supplier_name,
    ...
    public function listVehicleInsurancePolicies(int $vehicleId, int $actorUserId = 0, bool $actorHasAllVehicles = true): array
    {
        if (!$this->tableExists(self::TBL_INSURANCE_POLICIES) || $vehicleId <= 0) return [];
        $sql = 'SELECT p.* FROM ' . self::TBL_INSURANCE_POLICIES . ' p WHERE p.vehicle_id = :vehicle_id AND p.deleted_at IS NULL';
        $params = ['vehicle_id' => $vehicleId];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = p.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
        }
        $sql .= ' ORDER BY p.valid_to DESC, p.id DESC';
        $stmt = $this->pdo->prepare($sql); $stmt->execute($params);
        return $stmt->fetchAll() ?: [];
    }

    public function createVehicleInsurancePolicy(array $payload): int
    {
    ...
    public function listVehicleClaims(int $vehicleId, int $actorUserId = 0, bool $actorHasAllVehicles = true): array
    {
        if (!$this->tableExists(self::TBL_CLAIMS) || $vehicleId <= 0) return [];
        $sql = 'SELECT c.* FROM ' . self::TBL_CLAIMS . ' c WHERE c.vehicle_id = :vehicle_id AND c.deleted_at IS NULL';
        $params = ['vehicle_id' => $vehicleId];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = c.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
        }
    ...
    public function createVehicleClaim(array $payload): int
    {
    ...
    public function listVehicleTires(int $vehicleId, int $actorUserId = 0, bool $actorHasAllVehicles = true): array
    {
        if (!$this->tableExists(self::TBL_VEHICLE_TIRES) || $vehicleId <= 0) return [];
        $sql = 'SELECT t.* FROM ' . self::TBL_VEHICLE_TIRES . ' t WHERE t.vehicle_id = :vehicle_id AND t.deleted_at IS NULL';
        $params = ['vehicle_id' => $vehicleId];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = t.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
        }
        $sql .= ' ORDER BY t.season_code, t.installed_at DESC, t.id DESC';
        $stmt = $this->pdo->prepare($sql); $stmt->execute($params);
        return $stmt->fetchAll() ?: [];
    }

    public function createVehicleTires(array $payload): int
    {
    ...
    public function listVehicleFunding(int $vehicleId, int $actorUserId = 0, bool $actorHasAllVehicles = true): array
    {
        if (!$this->tableExists(self::TBL_VEHICLE_FUNDING) || $vehicleId <= 0) return [];
        $sql = 'SELECT f.* FROM ' . self::TBL_VEHICLE_FUNDING . ' f WHERE f.vehicle_id = :vehicle_id AND f.deleted_at IS NULL';
        $params = ['vehicle_id' => $vehicleId];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = f.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
        }
        $sql .= ' ORDER BY f.award_date DESC, f.id DESC';
        $stmt = $this->pdo->prepare($sql); $stmt->execute($params);
        return $stmt->fetchAll() ?: [];
    }

    public function createVehicleFunding(array $payload): int
    {
    ...
    public function findVehicleServiceRecordById(int $id, int $actorUserId = 0, bool $actorHasAllVehicles = true): ?array
    {
        if ($id <= 0) return null;
        $sql = 'SELECT s.* FROM ' . self::TBL_SERVICE_RECORDS . ' s WHERE s.id = :id AND s.deleted_at IS NULL';
        $params = ['id' => $id];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = s.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
        }
        $stmt = $this->pdo->prepare($sql . ' LIMIT 1');
        $stmt->execute($params);
    ...
    public function updateVehicleServiceRecord(int $id, array $payload, int $actorUserId, bool $actorHasAllVehicles = true): ?array
    {
    ...
    public function softDeleteVehicleServiceRecord(int $id, int $actorUserId, bool $actorHasAllVehicles = true): bool
    {
        $existing = $this->findVehicleServiceRecordById($id, $actorUserId, $actorHasAllVehicles);
    ...
    {
        if ($id <= 0) return null;
        $sql = 'SELECT e.* FROM ' . self::TBL_VEHICLE_EQUIPMENT . ' e WHERE e.id = :id AND e.deleted_at IS NULL';
        $params = ['id' => $id];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = e.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
        }
        $stmt = $this->pdo->prepare($sql . ' LIMIT 1');
        $stmt->execute($params);
    ...
    public function updateVehicleEquipment(int $id, array $payload, int $actorUserId, bool $actorHasAllVehicles = true): ?array
    {
    ...
    public function softDeleteVehicleEquipment(int $id, int $actorUserId, bool $actorHasAllVehicles = true): bool
    {
        $existing = $this->findVehicleEquipmentById($id, $actorUserId, $actorHasAllVehicles);
        if ($existing === null) return false;
        $stmt = $this->pdo->prepare('UPDATE ' . self::TBL_VEHICLE_EQUIPMENT . ' SET deleted_at = CURRENT_TIMESTAMP, updated_by_user_id = :updated_by_user_id, updated_at = CURRENT_TIMESTAMP WHERE id = :id AND deleted_at IS NULL');
        $stmt->execute(['id' => $id, 'updated_by_user_id' => $actorUserId > 0 ? $actorUserId : null]);
        $this->appendVehicleAuditEvent(['vehicle_id' => (int) $existing['vehicle_id'], 'event_type' => 'equipment_deleted', 'field_name' => 'equipment', 'old_value' => (string) $id, 'actor_user_id' => $actorUserId, 'actor_type' => 'user', 'source' => 'v2_equipment']);
        return $stmt->rowCount() > 0;
    }

    public function findVehicleInsurancePolicyById(int $id, int $actorUserId = 0, bool $actorHasAllVehicles = true): ?array
    {
        if ($id <= 0) return null;
        $sql = 'SELECT p.* FROM ' . self::TBL_INSURANCE_POLICIES . ' p WHERE p.id = :id AND p.deleted_at IS NULL';
        $params = ['id' => $id];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = p.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
    ...
    public function updateVehicleInsurancePolicy(int $id, array $payload, int $actorUserId, bool $actorHasAllVehicles = true): ?array
    {
    ...
    public function softDeleteVehicleInsurancePolicy(int $id, int $actorUserId, bool $actorHasAllVehicles = true): bool
    {
        $existing = $this->findVehicleInsurancePolicyById($id, $actorUserId, $actorHasAllVehicles);
        if ($existing === null) return false;
        $stmt = $this->pdo->prepare('UPDATE ' . self::TBL_INSURANCE_POLICIES . ' SET deleted_at = CURRENT_TIMESTAMP, updated_by_user_id = :updated_by_user_id, updated_at = CURRENT_TIMESTAMP WHERE id = :id AND deleted_at IS NULL');
        $stmt->execute(['id' => $id, 'updated_by_user_id' => $actorUserId > 0 ? $actorUserId : null]);
        $this->appendVehicleAuditEvent(['vehicle_id' => (int) $existing['vehicle_id'], 'event_type' => 'insurance_policy_deleted', 'field_name' => 'insurance_policy', 'old_value' => (string) $id, 'actor_user_id' => $actorUserId, 'actor_type' => 'user', 'source' => 'v2_insurance']);
        return $stmt->rowCount() > 0;
    }

    public function findVehicleClaimById(int $id, int $actorUserId = 0, bool $actorHasAllVehicles = true): ?array
    {
        if ($id <= 0) return null;
        $sql = 'SELECT c.* FROM ' . self::TBL_CLAIMS . ' c WHERE c.id = :id AND c.deleted_at IS NULL';
        $params = ['id' => $id];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = c.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
    ...
    public function updateVehicleClaim(int $id, array $payload, int $actorUserId, bool $actorHasAllVehicles = true): ?array
    {
    ...
    public function softDeleteVehicleClaim(int $id, int $actorUserId, bool $actorHasAllVehicles = true): bool
    {
        $existing = $this->findVehicleClaimById($id, $actorUserId, $actorHasAllVehicles);
        if ($existing === null) return false;
        $stmt = $this->pdo->prepare('UPDATE ' . self::TBL_CLAIMS . ' SET deleted_at = CURRENT_TIMESTAMP, updated_by_user_id = :updated_by_user_id, updated_at = CURRENT_TIMESTAMP WHERE id = :id AND deleted_at IS NULL');
        $stmt->execute(['id' => $id, 'updated_by_user_id' => $actorUserId > 0 ? $actorUserId : null]);
        $this->appendVehicleAuditEvent(['vehicle_id' => (int) $existing['vehicle_id'], 'event_type' => 'claim_deleted', 'field_name' => 'claim', 'old_value' => (string) $id, 'actor_user_id' => $actorUserId, 'actor_type' => 'user', 'source' => 'v2_claims']);
        return $stmt->rowCount() > 0;
    }

    public function findVehicleTiresById(int $id, int $actorUserId = 0, bool $actorHasAllVehicles = true): ?array
    {
        if ($id <= 0) return null;
        $sql = 'SELECT t.* FROM ' . self::TBL_VEHICLE_TIRES . ' t WHERE t.id = :id AND t.deleted_at IS NULL';
        $params = ['id' => $id];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = t.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
    ...
    public function updateVehicleTires(int $id, array $payload, int $actorUserId, bool $actorHasAllVehicles = true): ?array
    {
    ...
    public function softDeleteVehicleTires(int $id, int $actorUserId, bool $actorHasAllVehicles = true): bool
    {
        $existing = $this->findVehicleTiresById($id, $actorUserId, $actorHasAllVehicles);
        if ($existing === null) return false;
        $stmt = $this->pdo->prepare('UPDATE ' . self::TBL_VEHICLE_TIRES . ' SET deleted_at = CURRENT_TIMESTAMP, updated_by_user_id = :updated_by_user_id, updated_at = CURRENT_TIMESTAMP WHERE id = :id AND deleted_at IS NULL');
        $stmt->execute(['id' => $id, 'updated_by_user_id' => $actorUserId > 0 ? $actorUserId : null]);
        $this->appendVehicleAuditEvent(['vehicle_id' => (int) $existing['vehicle_id'], 'event_type' => 'tires_deleted', 'field_name' => 'tires', 'old_value' => (string) $id, 'actor_user_id' => $actorUserId, 'actor_type' => 'user', 'source' => 'v2_tires']);
        return $stmt->rowCount() > 0;
    }

    public function findVehicleFundingById(int $id, int $actorUserId = 0, bool $actorHasAllVehicles = true): ?array
    {
        if ($id <= 0) return null;
        $sql = 'SELECT f.* FROM ' . self::TBL_VEHICLE_FUNDING . ' f WHERE f.id = :id AND f.deleted_at IS NULL';
        $params = ['id' => $id];
        if ($actorUserId > 0 && !$actorHasAllVehicles) {
            $sql .= ' AND EXISTS (SELECT 1 FROM ' . self::TBL_ASSIGNMENTS . ' uva WHERE uva.vehicle_id = f.vehicle_id AND uva.user_id = :access_user_id)';
            $params['access_user_id'] = $actorUserId;
    ...
    public function updateVehicleFunding(int $id, array $payload, int $actorUserId, bool $actorHasAllVehicles = true): ?array
    {
    ...
    public function softDeleteVehicleFunding(int $id, int $actorUserId, bool $actorHasAllVehicles = true): bool
    {
        $existing = $this->findVehicleFundingById($id, $actorUserId, $actorHasAllVehicles);
        if ($existing === null) return false;
