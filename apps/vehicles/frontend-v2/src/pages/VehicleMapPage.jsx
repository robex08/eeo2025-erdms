...
import { fetchDashboardMetrics, fetchStationAddresses, fetchVehicleServiceRecords, fetchVehicles, triggerQuickSync } from '../services/apiClient';
...
function readCachedGeocodeByQuery(query) {
  ...
  const cacheKey = `vehicles_v2_station_geocode_v2_${normalizeLocationToken(query)}`;
  try {
...
function getAddressNumber(value) {
  const match = String(value || '').trim().match(/(?:^|\s)(\d+[A-Za-z]?)(?:\/\d+)?\s*$/u);
...
function findPhotonAddressFeature(features, station) {
  const stationStreet = normalizeAddressPart(String(station?.ulice || '').replace(/\s+\d+[A-Za-z]?(?:\/\d+)?\s*$/u, ''));
  const stationNumber = getAddressNumber(station?.ulice);
  if (!stationStreet || !stationNumber) {
    return Array.isArray(features) ? features[0] : null;
  }

  return (Array.isArray(features) ? features : []).find((feature) => {
    const properties = feature?.properties || {};
    return normalizeAddressPart(properties.street) === stationStreet
      && String(properties.housenumber || '').trim().toLowerCase() === stationNumber;
  }) || null;
}

async function geocodeViaPhoton(query, stationId, station = null) {
  ...
  try {
    ...
    logMapDebug('Photon request failed', { id: stationId, query, error });
    return null;
  }
  ...
  const feature = findPhotonAddressFeature(data?.features, station);
...
async function geocodeStationByAddress(station) {
  ...
  for (const query of queries) {
    ...
    const photon = await geocodeViaPhoton(query, station?.id, station);
    if (photon) {
      try {
        localStorage.setItem(cacheKey, JSON.stringify({ lat: photon.lat, lng: photon.lng }));
      } catch {
...
export default function VehicleMapPage() {
  ...
  async function loadServiceHistoryCount(vehicleId) {
    const id = Number(vehicleId || 0);
    if (id <= 0) {
      return 0;
    }

    if (stationServiceHistoryCountCacheRef.current.has(String(id))) {
      return Number(stationServiceHistoryCountCacheRef.current.get(String(id)) || 0);
    }

    if (serviceHistoryCacheRef.current.has(String(id))) {
      const cachedOrders = serviceHistoryCacheRef.current.get(String(id));
      const cachedCount = Array.isArray(cachedOrders) ? cachedOrders.length : 0;
      stationServiceHistoryCountCacheRef.current.set(String(id), cachedCount);
      return cachedCount;
    }

    try {
      const response = await fetchVehicleServiceRecords(id);
      const records = Array.isArray(response?.data?.items) ? response.data.items : [];
      const ordersRaw = records.map((record) => ({
        id: record.id,
      ...
      serviceHistoryCacheRef.current.set(String(id), ordersRaw);
      stationServiceHistoryCountCacheRef.current.set(String(id), count);
      return count;
    } catch {
      stationServiceHistoryCountCacheRef.current.set(String(id), 0);
      return 0;
    }
  }
  ...
  async function handleStationItemHover(event, station, stationVehicles) {
    ...
    const rows = await Promise.all(stationItems.map(async (vehicle) => {
      ...
      const serviceCount = await loadServiceHistoryCount(vehicle?.id);
      return {
