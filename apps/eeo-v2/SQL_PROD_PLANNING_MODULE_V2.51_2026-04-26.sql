-- ============================================================================
-- PLANNING MODUL - KONSOLIDOVANÁ MIGRACE PRO PROD
-- ============================================================================
-- Datum: 26. dubna 2026
-- Databáze: eeo2025 (PRODUCTION)
-- Účel: Kompletní implementace Planning modulu (plánování a rezervační kalendář)
-- 
-- ⚠️ PREREKVIZITY:
-- - Backup PROD databáze vytvořen
-- - Spuštěno po dohodě s vývojovým týmem
--
-- 📦 CO OBSAHUJE:
...
-- Tabulka 7: Rezervační termíny s kapacitou
CREATE TABLE IF NOT EXISTS `25_plan_udalosti_terminy` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `udalost_id` INT(11) NOT NULL COMMENT 'ID události (FK 25_plan_udalosti)',
  `nazev` VARCHAR(255) NULL COMMENT 'Název termínu (volitelné)',
  `dt_termin` DATETIME NOT NULL COMMENT 'Datum a čas termínu',
  `max_kapacita` INT(11) DEFAULT NULL COMMENT 'Maximální kapacita (NULL = neomezeno)',
  `obsazeno` INT(11) DEFAULT 0 COMMENT 'Počet obsazených míst',
  `deadline_uzavreni` DATETIME NULL COMMENT 'Deadline pro uzavření registrace (6h před)',
  `aktivni` TINYINT(1) DEFAULT 1 COMMENT 'Aktivní termín',
  `dt_created` DATETIME NOT NULL COMMENT 'Datum vytvoření',
  PRIMARY KEY (`id`),
  KEY `idx_udalost` (`udalost_id`),
  KEY `idx_termin` (`dt_termin`),
  KEY `idx_aktivni` (`aktivni`),
  CONSTRAINT `fk_terminy_udalost` FOREIGN KEY (`udalost_id`) REFERENCES `25_plan_udalosti` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Rezervační termíny s kapacitou';
...
-- Přiřazení práva k roli SUPERADMIN
...
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25_notifikace_typy_udalosti WHERE kod = 'PLANNING_MESSAGE_CREATED');

INSERT INTO 25_notifikace_typy_udalosti (kod, nazev, kategorie, popis, modul, aktivni)
SELECT 'PLANNING_MESSAGE_RESPONSE', 'Odpověď na zprávu', 'planning', 'Uživatel odpověděl na zprávu', 'planning', 1
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25_notifikace_typy_udalosti WHERE kod = 'PLANNING_MESSAGE_RESPONSE');

INSERT INTO 25_notifikace_typy_udalosti (kod, nazev, kategorie, popis, modul, aktivni)
SELECT 'PLANNING_EVENT_CREATED', 'Nová plánovaná událost', 'planning', 'Vytvoření nové události v modulu plánování', 'planning', 1
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25_notifikace_typy_udalosti WHERE kod = 'PLANNING_EVENT_CREATED');

INSERT INTO 25_notifikace_typy_udalosti (kod, nazev, kategorie, popis, modul, aktivni)
SELECT 'PLANNING_EVENT_RESPONSE', 'Odpověď na událost', 'planning', 'Uživatel odpověděl na plánovanou událost', 'planning', 1
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25_notifikace_typy_udalosti WHERE kod = 'PLANNING_EVENT_RESPONSE');

-- ============================================================================
-- ČÁST 4: NOTIFIKAČNÍ ŠABLONY
-- ============================================================================

-- Šablona pro novou zprávu
INSERT INTO 25_notifikace_sablony (typ, nazev, email_predmet, email_telo, email_vychozi, app_nadpis, app_zprava, priorita_vychozi, aktivni)
SELECT 'PLANNING_MESSAGE_CREATED', 'Nová zpráva na dashboardu',
  NULL, NULL, 0,
  'Nová zpráva: {{message_title}}',
  '{{author_name}} přidal zprávu na dashboard: "{{message_preview}}"',
  'normal', 1
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25_notifikace_sablony WHERE typ = 'PLANNING_MESSAGE_CREATED');

-- Šablona pro novou událost
INSERT INTO 25_notifikace_sablony (typ, nazev, email_predmet, email_telo, email_vychozi, app_nadpis, app_zprava, priorita_vychozi, aktivni)
SELECT 'PLANNING_EVENT_CREATED', 'Nová plánovaná událost',
  NULL, NULL, 0,
  'Nová událost: {{event_title}}',
  '{{author_name}} vytvořil událost "{{event_title}}" na {{event_date}}',
  'normal', 1
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25_notifikace_sablony WHERE typ = 'PLANNING_EVENT_CREATED');

-- ============================================================================
-- ČÁST 5: GLOBÁLNÍ NASTAVENÍ
-- ============================================================================

INSERT INTO 25a_nastaveni_globalni (klic, hodnota, popis, vytvoreno, aktualizovano)
SELECT 'PLANNING_USE_HIERARCHY', '0', 'Použít org. hierarchii pro cílení planning zpráv a událostí (0=vypnuto, 1=zapnuto)', NOW(), NOW()
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25a_nastaveni_globalni WHERE klic = 'PLANNING_USE_HIERARCHY');

INSERT INTO 25a_nastaveni_globalni (klic, hodnota, popis, vytvoreno, aktualizovano)
SELECT 'PLANNING_HIERARCHY_PROFILE_ID', NULL, 'ID hierarchického profilu pro planning modul (NULL = použít výchozí)', NOW(), NOW()
FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM 25a_nastaveni_globalni WHERE klic = 'PLANNING_HIERARCHY_PROFILE_ID');
...
WHERE TABLE_SCHEMA = 'eeo2025' 
  AND TABLE_NAME LIKE '25_plan%'
ORDER BY TABLE_NAME;

-- Zobrazit oprávnění
SELECT kod_prava, popis FROM 25_prava WHERE kod_prava = 'PLANNING_MANAGE';

-- Zobrazit event typy
SELECT kod, nazev FROM 25_notifikace_typy_udalosti WHERE kod LIKE '%PLANNING%';

-- Zobrazit globální nastavení
SELECT klic, hodnota FROM 25a_nastaveni_globalni WHERE klic LIKE '%PLANNING%';
...
-- - Zkontrolovat výstupy SELECT queries výše
-- - Rollback: Obnovit DB backup
--
-- ============================================================================
