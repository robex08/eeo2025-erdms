...
const Tr = styled.tr`
  ...
  /* Pokud je nastavena $bgColor prop, použít ji, jinak striping pattern */
...
export default function StatsReportsPage() {
  ...
  const [cashbookEntriesLoading, setCashbookEntriesLoading] = useState(false);
  ...
  const [cashbookPdfSelectedBook, setCashbookPdfSelectedBook] = useState(() => {
    try {
      const raw = localStorage.getItem(`${LOCAL_STORAGE_PREFIX}_cashbook_pdf_selected_book`);
      return raw || 'all';
    } catch (e) {}
    return 'all';
  });
  ...
  const [cashbookEntriesFilters, setCashbookEntriesFilters] = useState(() => {
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw);
        // Zajistit zpětnou kompatibilitu - doplnit typ, pokud chybí
        if (!parsed.typ) parsed.typ = 'all';
        return parsed;
      }
    } catch (e) {}
    return { 
      ...
      typ: 'all' // 'all' | 'prijem' | 'vydaj'
    };
  });
  const [cashbookEntriesSearch, setCashbookEntriesSearch] = useState('');
  const [cashbookEntriesSearchActive, setCashbookEntriesSearchActive] = useState('');
  ...
  const loadCashbookData = useCallback(async () => {
    ...
    setCashbookEntriesLoading(true);
    ...
    try {
      ...
      if (response.status === 'ok' && response.data) {
        ...
        // Načíst položky všech knih pro grafy a detail tabulku
        const allKnihaIds = [];
        ...
        // Načíst všechny entries paralelně
        if (allKnihaIds.length > 0) {
          await Promise.all(allKnihaIds.map(async (knihaId) => {
            try {
              const entriesResp = await getCashbookOverviewEntries({
                username,
                token,
                kniha_id: knihaId,
                page: 1,
              ...
              if (entriesResp.status === 'ok' && entriesResp.data) {
                setCashbookEntries(prev => ({
                  ...prev,
                  [knihaId]: entriesResp.data.entries || []
                }));
              }
            } catch (e) {
              console.error(`Chyba při načítání položek knihy ${knihaId}:`, e);
            }
          }));
        }
        
        setCashbookEntriesLoading(false);
      } else {
  ...
  // Hlavička stránky (Objednávky/Faktury/Aktivní smlouvy + Financování)
  // musí mít data i když uživatel otevře přímo tab mimo gate flow (cashbook/attachments/dohadne).
  const loadHeaderOverviewData = useCallback(async (forceReload = false) => {
    if (!token || !username) return;

    const needsOrders = forceReload ? true : orders.length === 0;
    const needsInvoices = forceReload ? true : invoices.length === 0;
    const needsContracts = forceReload ? true : contracts.length === 0;

    if (!needsOrders && !needsInvoices && !needsContracts) return;

    const tasks = [
      needsOrders ? loadOrders({ source: 'header' }) : Promise.resolve(null),
      needsInvoices ? loadInvoices({ source: 'header' }) : Promise.resolve(null),
      needsContracts ? loadContracts() : Promise.resolve(null),
    ];

    const [ordersResult, invoicesResult, contractsResult] = await Promise.all(tasks);
    ...
    if (ordersResult || invoicesResult) {
      setDataMeta(prev => ({
        ...prev,
        loadedAt: new Date().toISOString(),
        truncated: prev.truncated || !!ordersResult?.truncated || !!invoicesResult?.truncated
      }));
    }
  }, [token, username, orders.length, invoices.length, contracts.length, loadOrders, loadInvoices, loadContracts]);
  ...
  const handleLoadData = useCallback(async () => {
    ...
    try {
      ...
      else if (currentTab === 'attachments' || currentTab === 'cashbook' || currentTab === 'dohadne') {
        // Tyto taby mají vlastní lazy loading, ale hlavičkové KPI musí být naplněné hned po F5.
        await loadHeaderOverviewData(true);
      }
  ...
  }, [token, username, activeTab, loadCommonData, loadControlTabData, loadStatsTabData, loadSpendTabData, loadReportsTabData, loadVzdelTabData, loadPivotTabData, loadHeaderOverviewData, progress]);
  ...
  // POZN: Toto je nyní součástí loadCashbookData, takže tento useEffect už není potřeba
  // useEffect(() => {
  ...
  const cashbookPdfBookOptions = useMemo(() => {
    if (!cashbookData?.books?.length) return [];

    const map = new Map();
    cashbookData.books.forEach((book) => {
      const key = String(book?.pokladna_id ?? book?.cislo_pokladny ?? '').trim();
      if (!key) return;
      if (map.has(key)) return;

      const cislo = book?.cislo_pokladny ?? '';
      const nazev = book?.pokladna_nazev || (cislo ? `Pokladna ${cislo}` : `Pokladna ${key}`);
    ...
    return Array.from(map.values()).sort((a, b) => {
      const aNum = Number(a.cislo) || 0;
      const bNum = Number(b.cislo) || 0;
      return aNum - bNum;
    });
  }, [cashbookData]);

  useEffect(() => {
    if (cashbookPdfSelectedBook === 'all') return;
    if (!cashbookPdfBookOptions.length) return;
    if (!cashbookPdfBookOptions.some((item) => item.value === cashbookPdfSelectedBook)) {
      setCashbookPdfSelectedBook('all');
    }
  }, [cashbookPdfBookOptions, cashbookPdfSelectedBook]);
  ...
  const cashbookFilteredEntries = useMemo(() => {
    ...
    // Filtr podle roku (bere se z globálního cashbookFilters.rok)
    const rok = cashbookFilters.rok || new Date().getFullYear();
    filtered = filtered.filter(e => e.rok === rok);
    
    // Filtr podle měsíce (bere se z globálního cashbookFilters.mesic)
    if (cashbookFilters.mesic) {
      filtered = filtered.filter(e => e.mesic === cashbookFilters.mesic);
    }
    ...
    if (cashbookEntriesFilters.typ === 'prijem') {
      filtered = filtered.filter(e => (Number(e.castka_prijem) || 0) > 0);
    } else if (cashbookEntriesFilters.typ === 'vydaj') {
      filtered = filtered.filter(e => (Number(e.castka_vydaj) || 0) > 0);
    }
    
    // Fulltext search (pokud aktivní) - bez diakritiky
    if (cashbookEntriesSearchActive) {
      const term = removeDiacritics(cashbookEntriesSearchActive.toLowerCase().trim());
      filtered = filtered.filter(e => {
        const searchableText = removeDiacritics([
          ...
          e.komu_od_koho,
          e.pokladna_nazev,
          String(e.castka_prijem || ''),
          String(e.castka_vydaj || ''),
          ...(e.lp_kody_array || [])
        ].join(' ').toLowerCase());
    ...
    // Sorting pomocí tableSorts
    const s = tableSorts['cashbookEntries'];
    if (s?.field) {
      const { field, dir } = s;
      filtered.sort((a, b) => {
        let valA = a[field];
        ...
        } else if (field === 'castka_prijem' || field === 'castka_vydaj' || field === 'zustatek') {
          valA = Number(valA) || 0;
          valB = Number(valB) || 0;
        } else if (field === 'lp_kody') {
          // Sortování podle LP kódů - spojíme je čárkami a porovnáme
          valA = (a.lp_kody_array || []).join(', ').toLowerCase();
          valB = (b.lp_kody_array || []).join(', ').toLowerCase();
    ...
    return filtered;
  }, [cashbookFlatEntries, cashbookEntriesFilters, cashbookFilters.rok, cashbookFilters.mesic, cashbookEntriesSearchActive, tableSorts, removeDiacritics]);
  ...
  const handleExportPdf_cashbookOverview = useCallback(async () => {
    try {
      ...
      const periodLabel = cashbookFilters.mesic
        ? new Date(cashbookFilters.rok, cashbookFilters.mesic - 1, 1).toLocaleDateString('cs-CZ', { month: 'long', year: 'numeric' })
      ...
      const getCurrentUserName = () => {
        const userDetailFullWithTitles = buildPersonNameWithTitles({
          titulPred: userDetail?.titul_pred,
          jmeno: userDetail?.jmeno,
          prijmeni: userDetail?.prijmeni,
          titulZa: userDetail?.titul_za,
        });
        const userFullWithTitles = buildPersonNameWithTitles({
          titulPred: user?.titul_pred,
          jmeno: user?.jmeno,
          prijmeni: user?.prijmeni,
          titulZa: user?.titul_za,
        });

        return (
          userDetailFullWithTitles
          || userFullWithTitles
          ||
          (fullName && String(fullName).trim())
          || (userDetail?.cele_jmeno && String(userDetail.cele_jmeno).trim())
          || (user?.cele_jmeno && String(user.cele_jmeno).trim())
          || username
          || ''
        );
      };
      ...
      // Autoritativní zdroj LP struktury z tabulky 25_limitovane_prisliby (endpoint /limitovane_prisliby).
      const lpRows = await fetchLimitovanePrisliby({ token, username, context: 'cashbook' });
      ...
      (Array.isArray(lpRows) ? lpRows : []).forEach((lp) => {
        ...
        lpMetaByCode.set(lpCode, {
          ...
          kategorie: String(lp?.kategorie || 'NEURCENO').trim() || 'NEURCENO',
          prikazce,
          userId: lp?.user_id ?? null,
        });
      });
      ...
      const resolveCanonicalLpCode = (rawCode) => {
        ...
        // Pokud v pokladně chodí varianty LPPT1/LPPT2..., mapuj je na kmen LPPT z číselníku.
        let bestPrefix = '';
      ...
      const preparedBy = getCurrentUserName();
      const preparedDate = new Date().toLocaleDateString('cs-CZ');
      ...
      for (const target of targets) {
        ...
        const providedAdvance = allEntries.reduce((sum, item) => sum + parseAmount(item?.entry?.castka_prijem), 0);

        const detailRows = [];
        allEntries.forEach(({ entry }) => {
          const entryDate = formatDateForPdf(entry?.datum_zapisu);
          const entryDateRaw = entry?.datum_zapisu || null;
          const purpose = String(entry?.obsah_zapisu || entry?.komu_od_koho || '').trim();
          const sourceKey = String(entry?.id || `${entry?.datum_zapisu || ''}_${entry?.cislo_dokladu || ''}`);

          if (Array.isArray(entry?.detail_items) && entry.detail_items.length > 0) {
            entry.detail_items.forEach((di) => {
              const amount = parseAmount(di?.castka);
              if (amount <= 0) return;
              const lpKod = String(di?.lp_kod || entry?.lp_kod || '').trim();
              detailRows.push({
                datum: entryDate,
                datumRaw: entryDateRaw,
                lpKod,
                ucel: purpose,
                castkaNum: amount,
                sourceKey,
          ...
          detailRows.push({
            datum: entryDate,
            datumRaw: entryDateRaw,
            lpKod: String(entry?.lp_kod || '').trim(),
            ucel: purpose,
            castkaNum: expense,
            sourceKey,
          });
        ...
        // Striktní členění dle LP tabulky: 1 PDF = 1 (user_id + kategorie).
        // Např. LPPT1/LPPT2/LPPT3/LPPT4 => jeden PDF pro kategorii LPPT.
        const groupedPdfData = new Map();
        detailRows.forEach((row) => {
          const lpCodeNorm = resolveCanonicalLpCode(row.lpKod);
          const lpMeta = lpMetaByCode.get(lpCodeNorm);
          if (!lpMeta) {
            skippedRowsWithoutLpMaster += 1;
            return;
          }
          const lpCode = lpMeta?.cisloLp || (lpCodeNorm || 'BEZ_LP');
          const kategorie = lpMeta?.kategorie || 'NEURCENO';
          const prikazce = lpMeta?.prikazce || 'Neurčeno';
          const groupKey = `${lpMeta.userId || 'nouser'}__${kategorie}`;
          if (!groupedPdfData.has(groupKey)) {
            groupedPdfData.set(groupKey, { kategorie, prikazce, userId: lpMeta.userId || null, rows: [] });
          }
          groupedPdfData.get(groupKey).rows.push({ ...row, lpKod: lpCode });
        });

        if (groupedPdfData.size === 0) continue;

        for (const [, group] of groupedPdfData.entries()) {
          const prikazceName = group.prikazce;
          const rowsForPrikazce = [...group.rows].sort((a, b) => {
            const aTime = a?.datumRaw ? new Date(a.datumRaw).getTime() : 0;
            const bTime = b?.datumRaw ? new Date(b.datumRaw).getTime() : 0;
            const safeATime = Number.isFinite(aTime) ? aTime : 0;
            const safeBTime = Number.isFinite(bTime) ? bTime : 0;
            if (safeATime !== safeBTime) return safeATime - safeBTime;
            return String(a?.sourceKey || '').localeCompare(String(b?.sourceKey || ''), 'cs-CZ');
          });
          const totalAmount = rowsForPrikazce.reduce((sum, row) => sum + (Number(row.castkaNum) || 0), 0);
          const toSettle = totalAmount - providedAdvance;
          const attachmentsCount = new Set(rowsForPrikazce.map((r) => r.sourceKey)).size;

          const pdfRows = rowsForPrikazce.map((row) => ({
            datum: row.datum,
            lpKod: row.lpKod,
            ucel: row.ucel,
            castka: fmtCurrency(row.castkaNum),
          }));
          ...
          const safeCategory = sanitizeFileName(group.kategorie || 'neurceno') || 'neurceno';
      ...
      if (!directoryHandle && pendingBrowserDownloads.length > 0) {
        if (pendingBrowserDownloads.length === 1) {
          ...
          const zip = new JSZip();
          pendingBrowserDownloads.forEach((file) => {
          ...
          const zipScope = cashbookPdfSelectedBook === 'all'
            ? 'vsechny-pokladny'
            : sanitizeFileName(targets[0]?.nazev || targets[0]?.value || 'pokladna');
          const zipName = `vytuctovani-drobneho-vydani-${zipScope}-${period}.zip`;
      ...
      if (skippedRowsWithoutLpMaster > 0) {
  ...
  const isCashbookPending = isCashbookTab && (cashbookLoading || cashbookEntriesLoading);
                  ...
                  <SectionCard id="section-spendBySmlouvy">
                    <SectionHeader>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <button
                          onClick={handleExportCsv_spendBySmlouvy}
                  ...
                  <SectionCard id="section-spendByCashbook">
                    <SectionHeader>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        {!cashbookLoading && !cashbookEntriesLoading && cashbookBooksToRender.length > 0 && (
                          <>
                            <ExpandAllBtn
                              onClick={() => {
                    ...
                    {cashbookData?.summary && !cashbookLoading && !cashbookEntriesLoading && (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', padding: cashbookBooksToRender.length > 0 ? '0.75rem' : 0 }}>
                      {(cashbookLoading || cashbookEntriesLoading) ? null : cashbookBooksToRender.length === 0 ? (
              ...
              <>
                  ...
                  <SectionCard id="section-cashbookOverview">
                    <SectionHeader>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <select
                          value={cashbookPdfSelectedBook}
                          onChange={(e) => setCashbookPdfSelectedBook(e.target.value)}
                          title="Vyber pokladnu pro PDF export"
                          style={{
                        ...
                        >
                          <option value="all">PDF: všechny pokladny</option>
                          {cashbookPdfBookOptions.map((item) => (
                            <option key={item.value} value={item.value}>
                              PDF: {item.nazev} (č. {item.cislo || item.value})
                            </option>
                          ))}
                        </select>
                    ...
                    <div style={{ padding: '1rem 1.5rem', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            const next = { ...cashbookFilters, mesic: e.target.value === 'all' ? null : Number(e.target.value) };
                            setCashbookFilters(next);
                            setCashbookEntriesPage(1); // Reset stránky v Detail položek
                    ...
                    {!cashbookLoading && !cashbookEntriesLoading && cashbookData?.books && cashbookData.books.length > 0 ? (
                      <TableWrapper style={{ margin: 0 }}>
                        <Table>
                          <thead>
                            <tr>
                                                        ...
                                                        <Table>
                                                          ...
                                                          <tbody>
                                                            {monthEntries
                                                              ...
                                                              .filter(({ idx }) => !cashbookNormSearch || cbMatchData?.entryMatchSets[month.kniha_id]?.has(idx))
                                                              .map(({ entry, idx }) => {
                                                              const isEntryMatch = cashbookNormSearch && cbMatchData?.entryMatchSets[month.kniha_id]?.has(idx);
                                                              const isPrijem = (entry.castka_prijem || 0) > 0;
                                                              const isVydaj = (entry.castka_vydaj || 0) > 0;
                                                              const bgColor = isEntryMatch ? '#fefce8' : isPrijem ? '#f0fdf4' : isVydaj ? '#fef2f2' : undefined;
                                                              return (
                                                ...
                                                <TableWrapper style={{ margin: 0 }}>
                                                  <Table>
                                                    ...
                                                    <tbody>
                                                      {entries
                                                        .map((entry, idx) => ({ entry, idx }))
                                                        .filter(({ idx }) => !cashbookNormSearch || cbMatchData?.entryMatchSets[book.kniha_id]?.has(idx))
                                                        .map(({ entry, idx }) => {
                ...
                {cashbookData?.books?.length > 0 && (() => {
                  ...
                  cashbookData.books.forEach(book => {
                    ...
                    if (!pokladnaMap[key]) pokladnaMap[key] = { label, prijmy: 0, vydaje: 0, cislo: book.cislo_pokladny };
                  ...
                  const donut1Opts = {
                    ...
                    onClick: (event, elements) => {
                      if (elements.length > 0) {
                        ...
                        const cisloPokladny = p1Entries[index].cislo;
                        // Nastavit filtr pouze na tuto pokladnu (zachovat LP kódy)
                        const next = { ...cashbookEntriesFilters, pokladny: [cisloPokladny] };
                        ...
                        // Scroll na Detail položek
                        setTimeout(() => {
                          const el = document.getElementById('section-cashbookEntries');
                  ...
                  const donut2Opts = {
                    ...
                    onClick: (event, elements) => {
                      if (elements.length > 0) {
                        const index = elements[0].index;
                        const lpKod = lpLabels[index];
                        // Nastavit filtr pouze na tento LP kód (zachovat pokladny)
                        const next = { ...cashbookEntriesFilters, lp_kody: [lpKod] };
                        setCashbookEntriesFilters(next);
                        setCashbookEntriesPage(1);
                        try { localStorage.setItem(`${LOCAL_STORAGE_PREFIX}_cashbook_entries_filters`, JSON.stringify(next)); } catch (_) {}
                        // Scroll na Detail položek
                      ...
                      <ChartGrid>
                        ...
                        <ChartCard>
                          ...
                          <div style={{ fontSize: '0.75rem', color: '#64748b', marginBottom: '0.5rem' }}>
                            ...
                            <span style={{ marginLeft: '0.5rem', color: '#3b82f6', fontWeight: 600 }}>• Kliknutím filtruj Seznam položek</span>
                          </div>
                ...
                {isBlockVisible('cashbook', 'cashbookEntries') && (() => {
                  ...
                  // Statistiky - konverze na čísla kvůli NaN + konečný stav
                  const stats = cashbookFilteredEntries.reduce((acc, e) => {
                    acc.prijmy += Number(e.castka_prijem) || 0;
                    acc.vydaje += Number(e.castka_vydaj) || 0;
                    return acc;
                  }, { prijmy: 0, vydaje: 0 });
                  
                  // Konečný stav - použít data z cashbookData.books stejně jako v Přehled pokladen
                  const konecnyStav = (() => {
                    if (!cashbookData?.books) return 0;
                    
                    // Filtr pokladen (pokud je aktivní)
                    const aktivniPokladny = cashbookEntriesFilters.pokladny.length > 0 
                      ? new Set(cashbookEntriesFilters.pokladny) 
                      : null;
                    
                    let suma = 0;
                    cashbookData.books.forEach(book => {
                      // Pokud je filtr pokladen aktivní, kontrolovat
                      if (aktivniPokladny && !aktivniPokladny.has(book.cislo_pokladny)) {
                        return; // přeskočit tuto pokladnu
                      }
                      
                      if (book.mesic && book.kniha_id) {
                        // Měsíční kniha - vzít přímo koncovy_stav
                        suma += Number(book.koncovy_stav) || 0;
                      } else if (book.mesice && Array.isArray(book.mesice)) {
                        // Roční agregace - vzít koncovy_stav z posledního měsíce
                        const posledniMesic = book.mesice.reduce((max, m) => 
                          (m.mesic > max.mesic) ? m : max
                        , book.mesice[0]);
                        suma += Number(posledniMesic?.koncovy_stav) || 0;
                      }
                    });
                    ...
                    <SectionCard id="section-cashbookEntries" style={{ marginTop: '1.25rem' }}>
                      <SectionHeader>
                        <SectionTitle>
                          ...
                          Seznam pokladních dokladů - všechny pokladny
                        </SectionTitle>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                          ...
                          <SectionBadge $tone="default">Konečný stav: {fmtCurrency(konecnyStav)}</SectionBadge>
                          <SectionBadge $tone="default">
                            {cashbookFilters.mesic 
                              ? `${new Date(2000, cashbookFilters.mesic - 1, 1).toLocaleDateString('cs-CZ', { month: 'long' })} ${cashbookFilters.rok}`.replace(/^\w/, c => c.toUpperCase())
                              : `Celý rok ${cashbookFilters.rok}`}
                          </SectionBadge>
                        </div>
                      </SectionHeader>

                      {/* Filtry: Období, Fulltext, Pokladny, LP kódy */}
                      <div style={{ padding: '1rem 1.5rem', background: '#f8fafc', borderBottom: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {/* Řádek 1: Období + Fulltext search */}
                        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1, minWidth: '260px' }}>
                            ...
                            <div style={{ position: 'relative', flex: 1 }}>
                              ...
                              <input
                                type="text"
                                value={cashbookEntriesSearch}
                                onChange={(e) => setCashbookEntriesSearch(e.target.value)}
                                onKeyDown={(e) => { if (e.key === 'Enter') { setCashbookEntriesSearchActive(cashbookEntriesSearch.trim()); setCashbookEntriesPage(1); } }}
                                placeholder="doklad, obsah, komu, částka, LP kód… (Enter)"
                                style={{ width: '100%', padding: '0.4rem 1.8rem 0.4rem 2rem', borderRadius: '6px', border: `1px solid ${cashbookEntriesSearchActive ? '#3b82f6' : '#cbd5e1'}`, fontSize: '0.8rem', boxSizing: 'border-box', outline: 'none', background: cashbookEntriesSearchActive ? '#eff6ff' : '#fff', transition: 'border-color 0.15s' }}
                              />
                              {cashbookEntriesSearch && (
                                <button
                                  onClick={() => { setCashbookEntriesSearch(''); setCashbookEntriesSearchActive(''); setCashbookEntriesPage(1); }}
                                  style={{ position: 'absolute', right: '0.5rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center' }}
                                  title="Vymazat hledání"
                                >
                                  <FontAwesomeIcon icon={faXmark} style={{ fontSize: '0.8rem' }} />
                          ...
                          <ClearFiltersBtn
                            onClick={() => {
                              ...
                              setCashbookEntriesFilters(next);
                              setCashbookEntriesSearch('');
                              setCashbookEntriesSearchActive('');
                              setCashbookEntriesPage(1);
                              try { localStorage.setItem(`${LOCAL_STORAGE_PREFIX}_cashbook_entries_filters`, JSON.stringify(next)); } catch (_) {}
                            }}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
                          ...
                          <div>
                            ...
                            <select
                              value={cashbookEntriesFilters.typ || 'all'}
                              onChange={(e) => {
                                const next = { ...cashbookEntriesFilters, typ: e.target.value };
                                setCashbookEntriesFilters(next);
                                setCashbookEntriesPage(1);
                                try { localStorage.setItem(`${LOCAL_STORAGE_PREFIX}_cashbook_entries_filters`, JSON.stringify(next)); } catch (_) {}
                              }}
                              style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.875rem', cursor: 'pointer', background: '#fff' }}
                            >
                        ...
                        <>
                          <TableWrapper style={{ margin: 0 }}>
                            <Table>
                              <thead>
                                <tr>
                                  <ThSort style={{ width: '100px' }} onClick={() => handleTableSort('cashbookEntries', 'datum_zapisu')}>
                                    Datum{sortIcon('cashbookEntries', 'datum_zapisu')}
                                  </ThSort>
                                  <ThSort style={{ width: '110px' }} onClick={() => handleTableSort('cashbookEntries', 'cislo_dokladu')}>
                                    Číslo dokladu{sortIcon('cashbookEntries', 'cislo_dokladu')}
                                  </ThSort>
                                  <ThSort style={{ width: '200px' }} onClick={() => handleTableSort('cashbookEntries', 'pokladna_nazev')}>
                                    Pokladna{sortIcon('cashbookEntries', 'pokladna_nazev')}
                                  </ThSort>
                                  <ThSort onClick={() => handleTableSort('cashbookEntries', 'obsah_zapisu')}>
                                    Obsah zápisu{sortIcon('cashbookEntries', 'obsah_zapisu')}
                                  </ThSort>
                                  <ThSort style={{ width: '140px' }} onClick={() => handleTableSort('cashbookEntries', 'komu_od_koho')}>
                                    Komu/Od koho{sortIcon('cashbookEntries', 'komu_od_koho')}
                                  </ThSort>
                                  <ThRSort style={{ color: '#15803d', width: '100px' }} onClick={() => handleTableSort('cashbookEntries', 'castka_prijem')}>
                                    Příjem{sortIcon('cashbookEntries', 'castka_prijem')}
                                  </ThRSort>
                                  <ThRSort style={{ color: '#b91c1c', width: '100px' }} onClick={() => handleTableSort('cashbookEntries', 'castka_vydaj')}>
                                    Výdaj{sortIcon('cashbookEntries', 'castka_vydaj')}
                                  </ThRSort>
                                  <ThSort style={{ width: '120px' }} onClick={() => handleTableSort('cashbookEntries', 'lp_kody')}>
                                    LP kódy{sortIcon('cashbookEntries', 'lp_kody')}
                                  </ThSort>
                                </tr>
                              </thead>
                              <tbody>
                                {pageEntries.map((entry, idx) => {
                                  // Podbarvení řádků: příjem = světle zelená, výdaj = světle červená
                                  const isPrijem = (entry.castka_prijem || 0) > 0;
                                  const isVydaj = (entry.castka_vydaj || 0) > 0;
                          ...
                          {totalPages > 1 && (
                            <div style={{ padding: '1rem 1.5rem', background: '#f8fafc', borderTop: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
                              <div style={{ fontSize: '0.875rem', color: '#64748b' }}>
                                Zobrazeno {startIdx + 1} - {Math.min(endIdx, totalEntries)} z {totalEntries.toLocaleString('cs-CZ')}
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <span style={{ fontSize: '0.875rem', color: '#64748b' }}>Zobrazit:</span>
                                <select
                                  ...
                                  style={{ padding: '0.4rem 0.6rem', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '0.875rem', cursor: 'pointer' }}
                                >
                                ...
                                <button
                                  ...
                                  style={{ padding: '0.4rem 0.6rem', borderRadius: '6px', border: '1px solid #cbd5e1', background: cashbookEntriesPage === 1 ? '#f1f5f9' : '#fff', cursor: cashbookEntriesPage === 1 ? 'not-allowed' : 'pointer', fontSize: '0.875rem', color: '#334155' }}
                                >
                                  ««
                                </button>
                                <button
                                ...
                                <span style={{ fontSize: '0.875rem', color: '#64748b', padding: '0 0.5rem' }}>Stránka {cashbookEntriesPage} z {totalPages}</span>
                                <button
                                  disabled={cashbookEntriesPage === totalPages}
                                  onClick={() => setCashbookEntriesPage(p => Math.min(totalPages, p + 1))}
                                  style={{ padding: '0.4rem 0.6rem', borderRadius: '6px', border: '1px solid #cbd5e1', background: cashbookEntriesPage === totalPages ? '#f1f5f9' : '#fff', cursor: cashbookEntriesPage === totalPages ? 'not-allowed' : 'pointer', fontSize: '0.875rem', color: '#334155' }}
                                >
                                  ›
                                </button>
                                <button
