  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const apiFilters = {
        ...
        show_inactive: filters.stav === 'NEAKTIVNI',  // ✅ FIX: Neaktivní JEN když explicitně vybrán stav NEAKTIVNI
                    ...
                    <tr style={{ background: '#f8fafc' }}>
                      <TableCell colSpan={columns.length} style={{ padding: '0.5rem 1rem 0.75rem 2rem', borderBottom: '2px solid #cbd5e1' }}>
                        {contractExpandLoading[row.original.id] ? (
                          ...
                          const czFormat = (v) => new Intl.NumberFormat('cs-CZ', { style: 'currency', currency: 'CZK', minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(v || 0);
