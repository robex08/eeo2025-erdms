...
import { X, Save, AlertCircle, Check, Calendar, Hash } from 'lucide-react';
...
const EditAssignmentDialog = ({
  ...
  useEffect(() => {
    if (isOpen && assignment) {
      setFormData({
        // ✅ Backend vrací ciselna_rada_vpd/ppd, ne vpd_cislo/ppd_cislo
        vpd_cislo: assignment.ciselna_rada_vpd || assignment.vpd_cislo || '',
        vpd_od_cislo: parseInt(assignment.vpd_od_cislo) || 1,
        ppd_cislo: assignment.ciselna_rada_ppd || assignment.ppd_cislo || '',
        ppd_od_cislo: parseInt(assignment.ppd_od_cislo) || 1,
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await cashbookAPI.updateAssignment(
        assignment.id,
        formData.vpd_cislo,
        parseInt(formData.vpd_od_cislo),
        formData.ppd_cislo,
        parseInt(formData.ppd_od_cislo),
        formData.platne_od || null,
        formData.platne_do || null
      );

      if (result.status === 'ok') {
        setSuccessMessage('✓ Přiřazení bylo úspěšně aktualizováno');
        setIsClosing(true);
        ...
        setErrorMessage(result.message || 'Nepodařilo se aktualizovat přiřazení');
      }
    } catch (error) {
      console.error('❌ Chyba při ukládání přiřazení:', error);
      setErrorMessage(error.message || 'Nepodařilo se aktualizovat přiřazení');
    } finally {
      if (!isClosing) {
        setLoading(false);
      }
    ...
    <ModalOverlay onClick={!loading ? onClose : undefined}>
      <ModalContainer onClick={(e) => e.stopPropagation()}>
        <ModalHeader>
          <ModalHeaderContent>
            <HeaderLeft>
              <ModalTitle>Upravit přiřazení pokladny</ModalTitle>
              <ModalSubtitle>
                Aktualizujte čísla VPD/PPD a období platnosti
              </ModalSubtitle>
            </HeaderLeft>
