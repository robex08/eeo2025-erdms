...
const OrderCommentsTooltip = ({
  ...
  const [isPositioned, setIsPositioned] = useState(false); // ✅ Flag pro zobrazení až po vypočítání pozice
  const [confirmDialog, setConfirmDialog] = useState({ isOpen: false, commentId: null });
  const containerRef = useRef(null);
  
  // ✅ Vypočítat pozici PŘED vykreslením pomocí useLayoutEffect (synchronně, žádný flickering!)
  useLayoutEffect(() => {
    if (iconRef?.current) {
      const iconRect = iconRef.current.getBoundingClientRect();
      const tooltipWidth = 400;
      const tooltipHeight = 500; // Odhadovaná výška (max-height)
      const padding = 10;
      ...
      setIsPositioned(true); // ✅ Povolit zobrazení
    }
  }, [iconRef]);
          ...
          {!loading && !error && comments.length > 0 && (
            <CommentsTimeline>
              {comments.map((comment) => {
                // ✅ Backend vrací user_id, ne autor_id
