...
$queries = array(
   ...
   // "get-orders-year" => "SELECT * FROM {$config['db']['objednavky']} WHERE datum_u BETWEEN :yearFrom AND :yearTo",            
    ...
    //  -------                
        ...
        "SELECT obj.id, obj.evidencni_c, DATE_FORMAT(obj.datum_u, '%d.%m.%Y') as datum_p, obj.dodatek_sml_id, obj.vypovedni_lhuta, g.garant, u.surname as uName, o.okres, um.umisteni, ds.druh as druh_sml, obj.tt_vyrc, obj.tt_dinfo, obj.faktura, obj.pokladni_dok, DATE_FORMAT(obj.dt_pridani, '%d.%m.%Y %H:%i:%s') as dt_pridani, TRIM(CONCAT(u.surname, ' ', u.name)) as userCreator, DATE_FORMAT(obj.dt_modifikace, '%d.%m.%Y %H:%i:%s') as dt_modifikace, TRIM(CONCAT(upd.surname, ' ', upd.name)) as userUpdater, obj.partner_nazev, obj.partner_ic, obj.partner_adresa, obj.obsah, obj.cena, obj.cena_rok, obj.platnost_do, obj.ukonceno, DATE_FORMAT(obj.dt_zverejneni, '%d.%m.%Y') as dt_zverejneni, obj.zverejnit, obj.idds, COALESCE(prilohy_count.cnt, 0) as prilohy, obj.poznamka, obj.poznamka_garant, objmd.metadata as objMetaData FROM :tab_obj obj LEFT JOIN garant g ON g.id = obj.garant_id LEFT JOIN users u ON u.id = obj.user_id LEFT JOIN okresy o ON o.id = obj.okres_id LEFT JOIN umisteni um ON um.id = obj.umisteni_id LEFT JOIN druh_smlouvy ds ON ds.id = obj.druh_sml_id LEFT JOIN users upd ON upd.id = obj.upd_user_id LEFT JOIN (SELECT id_smlouvy, COUNT(*) as cnt FROM :tbl_oprilohy GROUP BY id_smlouvy) prilohy_count ON prilohy_count.id_smlouvy = obj.id LEFT JOIN :tbl_objMD objmd ON objmd.objednavka_id = obj.id WHERE obj.datum_u >= :yearFrom AND obj.datum_u < DATE_ADD(:yearTo, INTERVAL 1 DAY);",
        "react-order-id" =>
