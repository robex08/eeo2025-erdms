...
$queries = array(
   ...
   // "get-orders-year" => "SELECT * FROM {$config['db']['objednavky']} WHERE datum_u BETWEEN :yearFrom AND :yearTo",            
    ...
    //  -------                
        ...
        "SELECT id, evidencni_c, DATE_FORMAT(datum_u, '%d.%m.%Y') as datum_p, dodatek_sml_id, vypovedni_lhuta, (SELECT garant FROM garant WHERE garant.id = garant_id) as garant, (SELECT surname FROM users WHERE users.id = user_id) as uName, (SELECT okres FROM okresy WHERE okresy.id = okres_id) as okres, (SELECT umisteni FROM umisteni WHERE umisteni.id = umisteni_id) as umisteni, (SELECT druh FROM druh_smlouvy WHERE druh_smlouvy.id = druh_sml_id) as druh_sml, tt_vyrc, tt_dinfo, faktura, pokladni_dok, DATE_FORMAT(dt_pridani, '%d.%m.%Y %H:%i:%s') as dt_pridani, (SELECT TRIM(CONCAT(surname,' ', name)) FROM users WHERE users.id = user_id) as userCreator, DATE_FORMAT(dt_modifikace, '%d.%m.%Y %H:%i:%s') as dt_modifikace, (SELECT TRIM(CONCAT(surname,' ', name)) FROM users WHERE users.id = upd_user_id) as userUpdater, partner_nazev, partner_ic, partner_adresa, obsah, cena, cena_rok, platnost_do, ukonceno, DATE_FORMAT(dt_zverejneni, '%d.%m.%Y') as dt_zverejneni, zverejnit, idds, (SELECT COUNT(id_smlouvy) FROM :tbl_oprilohy WHERE :tbl_oprilohy.id_smlouvy = :tab_obj.id) as prilohy, poznamka, poznamka_garant, (SELECT metadata FROM :tbl_objMD WHERE :tbl_objMD.objednavka_id = :tab_obj.id) as objMetaData FROM :tab_obj WHERE datum_u >= :yearFrom AND datum_u < DATE_ADD(:yearTo, INTERVAL 1 DAY);",
        "react-order-id" =>
