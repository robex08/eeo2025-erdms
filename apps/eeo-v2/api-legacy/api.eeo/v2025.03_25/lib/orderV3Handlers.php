...
/**
 * Pomocná funkce: extrahuje kódy středisek z hodnoty (string/JSON/array)
 * @param mixed $value
 * @return array
 */
function extractStrediskaCodes($value) {
    ...
    // Odstranit prázdné a duplicitní
...
/**
 * Pomocná funkce: sloučí dvě pole kódů středisek (unique)
 * @param array $base
 * @param array $add
 * @return array
 */
function mergeStrediskaCodes($base, $add) {
 ...
 * Přehled objednávek typu MAJETEK (druh_objednavky atribut_objektu=1)
 * s workflow od ROZPRACOVANA a dál, včetně středisek, faktur a umístění.
 */
function handle_order_v3_majetek_list($input, $config, $queries) {
    ...
    try {
        ...
        // Druh objednávky = MAJETEK (atribut_objektu = 1)
        $where_conditions[] = "cs.atribut_objektu = 1";

        // Aplikovat permissions
        applyOrderV3UserPermissions($user_id, $db, $where_conditions, $where_params);

        // Období
        $period_range = calculatePeriodRange($period);
        if ($period_range) {
            $where_conditions[] = "o.dt_objednavky >= ?";
            $where_params[] = $period_range['date_from'];
            $where_conditions[] = "o.dt_objednavky <= ?";
            $where_params[] = $period_range['date_to'];
        }

        // Workflow stav - výchozí ROZPRACOVANA a dál
        ...
        // Druh objednávky kód - JSON/string fallback
        $druh_kod_expr = "NULLIF((CASE 
            WHEN JSON_VALID(o.druh_objednavky_kod) THEN COALESCE(
                JSON_UNQUOTE(JSON_EXTRACT(o.druh_objednavky_kod, '$.kod_stavu')),
                JSON_UNQUOTE(JSON_EXTRACT(o.druh_objednavky_kod, '$.kod'))
            )
        ...
        $sql_count = "
            ...
            LEFT JOIN " . TBL_DODAVATELE . " d ON o.dodavatel_id = d.id
            LEFT JOIN " . TBL_CISELNIK_STAVY . " cs
                ON cs.typ_objektu = 'DRUH_OBJEDNAVKY'
                AND cs.kod_stavu = $druh_kod_expr
            WHERE $where_sql
        ";
        ...
        $sql_orders = "
            SELECT 
                ...
                o.max_cena_s_dph,
                COALESCE(o.dodavatel_nazev, d.nazev) as dodavatel_nazev,
                cs.nazev_stavu as druh_objednavky_nazev,
                cs.atribut_objektu as druh_objednavky_atribut,
                (SELECT COUNT(*) FROM " . TBL_FAKTURY . " f WHERE f.objednavka_id = o.id AND f.aktivni = 1) as pocet_faktur,
                (SELECT COALESCE(SUM(f.fa_castka), 0) FROM " . TBL_FAKTURY . " f WHERE f.objednavka_id = o.id AND f.aktivni = 1) as faktury_celkova_castka_s_dph,
        ...
        // Dodatečná data: umístění z položek a střediska z faktur
        $order_ids = array_column($orders, 'id');
        $items_map = array();
        $invoice_strediska_map = array();
        ...
        if (!empty($order_ids)) {
            ...
            // Umístění z položek
            $sql_items = "
                SELECT p.objednavka_id, p.usek_kod, p.budova_kod, p.mistnost_kod, p.poznamka
            ...
            while ($row = $stmt_items->fetch(PDO::FETCH_ASSOC)) {
                $oid = (int)$row['objednavka_id'];
                if (!isset($items_map[$oid])) {
                    $items_map[$oid] = array();
                }
                $items_map[$oid][] = array(
                    'usek_kod' => $row['usek_kod'],
                    'budova_kod' => $row['budova_kod'],
                    'mistnost_kod' => $row['mistnost_kod'],
                    'poznamka' => $row['poznamka']
                );
            }

            // Střediska z faktur (priorita)
            $sql_faktury = "
                SELECT f.objednavka_id, f.fa_strediska_kod
                FROM " . TBL_FAKTURY . " f
                WHERE f.objednavka_id IN ($placeholders) AND f.aktivni = 1
            ";
            $stmt_faktury = $db->prepare($sql_faktury);
            $stmt_faktury->execute($order_ids);
            while ($row = $stmt_faktury->fetch(PDO::FETCH_ASSOC)) {
                $oid = (int)$row['objednavka_id'];
                $codes = extractStrediskaCodes($row['fa_strediska_kod']);
                if (!isset($invoice_strediska_map[$oid])) {
                    $invoice_strediska_map[$oid] = array();
                }
                $invoice_strediska_map[$oid] = mergeStrediskaCodes($invoice_strediska_map[$oid], $codes);
            }
        }
        ...
        foreach ($orders as &$order) {
            ...
            // Umístění (JSON array z položek)
            $order['umisteni_polozky'] = $items_map[$order_id] ?? array();

            // Střediska: priorita faktury, jinak objednávka
            $strediska_codes = array();
            if (!empty($invoice_strediska_map[$order_id])) {
                $strediska_codes = $invoice_strediska_map[$order_id];
                $order['strediska_zdroj'] = 'FAKTURA';
            } else {
                $strediska_codes = extractStrediskaCodes($order['strediska_kod'] ?? null);
                $order['strediska_zdroj'] = 'OBJEDNAVKA';
            }

            $order['strediska_kod'] = $strediska_codes;
            $strediska_enriched = !empty($strediska_codes) ? loadStrediskaByKod($db, $strediska_codes) : array();
        ...
        http_response_code(500);
        echo json_encode(array('status' => 'error', 'message' => 'Chyba při načítání přehledu majetku'));
    }
}
...
function getOrderStatsWithPeriod($db, $period, $user_id = 0, $filtered_where_sql = null, $filtered_where_params = array()) {
    ...
    // Celková cena s DPH + částky pro stavové souhrny
    $sql_total_amount = "
        SELECT 
            COALESCE(SUM(
                CASE
            ...
            COALESCE(SUM(
                CASE
                    WHEN JSON_UNQUOTE(JSON_EXTRACT(o.stav_workflow_kod, CONCAT('$[', JSON_LENGTH(o.stav_workflow_kod) - 1, ']'))) IN (
                        'ROZPRACOVANA', 'ODESLANA', 'POTVRZENA', 'FAKTURACE', 'VECNA_SPRAVNOST', 'ZKONTROLOVANA'
                    )
                    THEN
                        CASE
                            WHEN (SELECT COALESCE(SUM(f.fa_castka), 0) 
    ...
    $stats['rozpracovaneAmount'] = floatval($amount_result['rozpracovane_amount']);
    $stats['dokoncenaAmount'] = floatval($amount_result['dokoncena_amount']);
