...
$queries['chat_stats_overview'] = "
    SELECT 
        (SELECT COUNT(*) FROM ".TABLE_CHAT_KONVERZACE." WHERE aktivni = 1) as pocet_konverzaci,
        (SELECT COUNT(*) FROM ".TABLE_CHAT_ZPRAVY." WHERE smazano = 0) as pocet_zprav,
        (SELECT COUNT(DISTINCT user_id) FROM ".TABLE_CHAT_UCASTNICI." WHERE aktivni = 1) as aktivnich_uzivatelu,
        (SELECT COUNT(*) FROM ".TABLE_CHAT_ONLINE_STATUS." WHERE status != 'OFFLINE') as online_uzivatelu
";
...
$queries['chat_stats_active_conversations'] = "
    ...
    FROM ".TABLE_CHAT_KONVERZACE." k
    LEFT JOIN ".TABLE_CHAT_ZPRAVY." z ON k.id = z.konverzace_id AND z.smazano = 0
    WHERE k.aktivni = 1
