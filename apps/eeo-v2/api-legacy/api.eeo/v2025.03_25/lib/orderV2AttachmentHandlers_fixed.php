...
function get_order_v2_upload_path($config, $objednavka_id, $user_id) {
    ...
    // Struktura adresářů - Order V2 má vlastní strukturu
    $subPath = 'order-v2/';
    
    // Podle data pro organizaci
    $subPath .= date('Y') . '/' . date('m') . '/';
    
    // Podle objednávky pro separaci
...
function validate_order_v2_file_upload($config, $file_data) {
    ...
    $maxFileSize = isset($uploadConfig['max_file_size']) ? 
        $uploadConfig['max_file_size'] : (5 * 1024 * 1024); // 5MB default
...
/**
 * Generování GUID pro systemový název souboru
 * PHP 5.6 compatible
 */
function generate_order_v2_file_guid() {
    // Generuj GUID s date prefixem
    $guid = sprintf(
...
function handle_order_v2_upload_attachment($input, $config, $queries) {
    ...
    if (is_string($order_id) && strpos($order_id, 'draft_') === 0) {
        http_response_code(400);
        echo json_encode(array('status' => 'error', 'message' => 'Nelze nahrávat přílohy k draft objednávce'));
        return;
    }
...
function handle_order_v2_list_attachments($input, $config, $queries) {
    ...
    try {
        ...
        $sql = "SELECT id, guid, typ_prilohy, originalni_nazev_souboru, 
                       velikost_souboru_b, systemova_cesta, dt_nahrano, nahrano_uzivatel_id
        ...
        foreach ($attachments as $attachment) {
            $result[] = array(
                ...
                'upload_date' => $attachment['dt_nahrano'],
                'uploaded_by_user_id' => (int)$attachment['nahrano_uzivatel_id'],
                'file_exists' => file_exists($attachment['systemova_cesta'])
            );
        }
