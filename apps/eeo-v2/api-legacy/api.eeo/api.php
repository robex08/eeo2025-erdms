...
require_once __DIR__ . '/v2025.03_25/lib/invoicesExportHandlers.php';
...
// AI CHAT - OpenRouter proxy (backend only)
require_once __DIR__ . '/v2025.03_25/lib/aiHandlers.php';
...
switch ($endpoint) {
    ...
    case 'substitution/manageable-users':
        ...
        break;
    case 'substitution/is-candidate':
        if ($request_method === 'POST') {
            $response = handle_substitution_is_candidate($input, $pdo);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode($response, JSON_UNESCAPED_UNICODE);
        } else {
            http_response_code(405);
    ...
    case 'ai/chat':
        if ($request_method === 'POST') {
            handle_ai_chat_proxy($input, $config, $queries);
        } else {
            http_response_code(405);
    ...
    case 'invoices25/export':
        if ($request_method === 'GET') {
            handle_invoices25_export($input, $config, $queries);
        } else {
            http_response_code(405);
            echo json_encode(array('err' => 'Metoda není povolena'));
        }
    ...
    case 'dashboard/data':
        if ($request_method === 'POST') {
            handle_dashboard_data($input, $config, $queries);
        } else {
            http_response_code(405);
            echo json_encode(array('status' => 'error', 'message' => 'Pouze POST metoda'));
        }
