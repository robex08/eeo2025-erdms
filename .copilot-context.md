# ERDMS - Project Context Memory

## 📋 Základní informace o projektu

### Název projektu
**ERDMS** - Elektronický Rozcestník pro Dokument Management System

### Organizace
**ZZS** - Zdravotnická záchranná služba

### Produkční doména
- **URL:** https://erdms.zachranka.cz
- **SSL:** ✅ Aktivní

---

## 📂 Struktura projektu

### Development workspace: `/var/www/eeo2025/`
```
eeo2025/
├── client/                     # React frontend (Vite + MSAL)
│   ├── src/
│   │   ├── components/
│   │   │   ├── HomePage.jsx/css
│   │   │   └── LoginPage.jsx/css
│   │   ├── config/
│   │   │   └── authConfig.js   # MSAL konfigurace pro React
│   │   ├── App.jsx
│   │   └── main.jsx
│   ├── package.json
│   ├── vite.config.js
│   └── .env                    # CLIENT_ID, TENANT_ID, API_SCOPE
│
├── server/                     # Express API (Node.js + MSAL)
│   ├── src/
│   │   ├── config/
│   │   │   └── msalConfig.js   # MSAL konfigurace pro server
│   │   ├── middleware/
│   │   │   └── authMiddleware.js
│   │   ├── routes/
│   │   │   ├── auth.js
│   │   │   └── protected.js
│   │   └── index.js            # Hlavní server soubor
│   ├── package.json
│   └── .env                    # CLIENT_ID, TENANT_ID, CLIENT_SECRET
│
└── docs/                       # Dokumentace
    ├── deployment/
    │   └── DEPLOYED.md
    ├── development/
    │   ├── START.md
    │   └── CHECKLIST.md
    ├── production/
    │   ├── PRODUCTION.md
    │   └── SYSTEMD_SERVICE.md
    └── setup/
        └── MICROSOFT_ENTRA_SETUP.md
```

### Production build: `/var/www/erdms/`
```
erdms/
├── index.html                  # Client build (SPA entry point)
├── assets/                     # JS, CSS bundly z Vite
├── logo-ZZS.png
├── vite.svg
├── MICROSOFT_ENTRA_SETUP.md
└── api/
    └── v1.0/                   # API deployment
        ├── src/
        │   ├── config/
        │   ├── middleware/
        │   ├── routes/
        │   └── index.js
        ├── node_modules/
        ├── package.json
        ├── .env
        ├── server.log
        └── server.pid
```

---

## 🔐 Autentizace - Microsoft Entra ID

### Technologie
- **Frontend:** `@azure/msal-react` (v3.0.22)
- **Backend:** `@azure/msal-node` (v3.8.3)

### Flow
1. User → Přihlášení přes Microsoft (SSO)
2. Frontend získá Access Token
3. Frontend → API s `Bearer Token` v hlavičce
4. Backend validuje token pomocí JWKS
5. Backend vrací data

### Konfigurace (.env)

**Client (.env):**
```env
VITE_AZURE_CLIENT_ID=...
VITE_AZURE_TENANT_ID=...
VITE_AZURE_AUTHORITY=https://login.microsoftonline.com/{TENANT_ID}
VITE_REDIRECT_URI=https://erdms.zachranka.cz
VITE_AZURE_API_SCOPE=api://{CLIENT_ID}/access_as_user
```

**Server (.env):**
```env
AZURE_CLIENT_ID=...
AZURE_TENANT_ID=...
AZURE_CLIENT_SECRET=...
AZURE_AUTHORITY=https://login.microsoftonline.com/{TENANT_ID}
AZURE_API_SCOPE=api://{CLIENT_ID}/access_as_user
PORT=5000
NODE_ENV=production
```

### Scopes
- **Login:** `User.Read`, `profile`, `email`, `openid`
- **API:** `api://{CLIENT_ID}/access_as_user`

---

## 🛠️ Technologie

### Frontend
- **React:** 19.2.0
- **Vite:** 7.2.4 (build tool)
- **React Router:** 7.9.6
- **Axios:** 1.13.2 (HTTP klient)
- **MSAL React:** 3.0.22

### Backend
- **Node.js:** 20.x
- **Express:** 5.2.1
- **MSAL Node:** 3.8.3
- **JWT:** jsonwebtoken 9.0.2, jwks-rsa 3.2.0
- **Security:** helmet 8.1.0, cors 2.8.5
- **Logger:** morgan 1.10.1

### Server
- **Web server:** Apache 2.4
- **OS:** Linux
- **Process manager:** nohup + PID file (možná upgrade na systemd)

---

## 🚀 Spuštění

### Development
```bash
# Terminal 1 - Server
cd /var/www/eeo2025/server
npm run dev                    # nodemon na portu 5000

# Terminal 2 - Client
cd /var/www/eeo2025/client
npm run dev                    # Vite dev server na portu 3000
```

### Production - API Server
```bash
# Status
ps aux | grep "node src/index.js"

# Start
cd /var/www/erdms/api/v1.0
source ~/.nvm/nvm.sh
nohup node src/index.js > /var/log/erdms-api.log 2>&1 &
echo $! > /var/run/erdms-api.pid

# Stop
kill $(cat /var/run/erdms-api.pid)

# Restart
kill $(cat /var/run/erdms-api.pid) && sleep 1
cd /var/www/erdms/api/v1.0 && nohup node src/index.js > /var/log/erdms-api.log 2>&1 &
echo $! > /var/run/erdms-api.pid

# Logy
tail -f /var/log/erdms-api.log
```

### Production - Apache
```bash
# Status
systemctl status apache2

# Restart
systemctl restart apache2

# Logy
tail -f /var/log/apache2/erdms-443-ssl-access.log
tail -f /var/log/apache2/erdms-443-ssl-error.log
```

---

## 📦 Build & Deploy workflow

### 1. Změny v kódu
```bash
# Edituj soubory v /var/www/eeo2025/client/ nebo /server/
```

### 2. Test v development
```bash
cd /var/www/eeo2025/client && npm run dev
cd /var/www/eeo2025/server && npm run dev
```

### 3. Build frontend
```bash
cd /var/www/eeo2025/client
npm run build                   # Vytvoří dist/
```

### 4. Deploy frontend
```bash
cp -r /var/www/eeo2025/client/dist/* /var/www/erdms/
```

### 5. Deploy backend
```bash
# Zkopíruj změněné soubory
cp /var/www/eeo2025/server/src/* /var/www/erdms/api/v1.0/src/

# Restart API
kill $(cat /var/run/erdms-api.pid)
cd /var/www/erdms/api/v1.0 && nohup node src/index.js > /var/log/erdms-api.log 2>&1 &
echo $! > /var/run/erdms-api.pid
```

### 6. Ověření
```bash
# Health check
curl http://localhost:5000/api/health

# Test v prohlížeči
firefox https://erdms.zachranka.cz
```

---

## 🌐 Apache VirtualHost konfigurace

**Soubor:** `/etc/apache2/sites-available/erdms.zachranka.cz.conf`

### Klíčové direktivy
- **DocumentRoot:** `/var/www/erdms/`
- **ServerName:** `erdms.zachranka.cz`
- **SSL:** ✅ Aktivní (certifikát Let's Encrypt)
- **API Proxy:** `/api → http://localhost:5000/api`
- **SPA Routing:** `FallbackResource /index.html`

### Proxy nastavení
```apache
ProxyPass /api http://localhost:5000/api
ProxyPassReverse /api http://localhost:5000/api
```

---

## 🔍 Důležité endpointy

### API
- **Health:** `GET http://localhost:5000/api/health`
- **Auth Token:** `POST http://localhost:5000/api/auth/token`
- **Protected Data:** `GET http://localhost:5000/api/protected/data`

### Frontend
- **Homepage:** `https://erdms.zachranka.cz/`
- **Login:** `https://erdms.zachranka.cz/` (HomePage zobrazí login)

---

## ⚠️ Aktuální stav

### ✅ Co funguje
- Apache VirtualHost konfigurace
- SSL certifikát
- Frontend build deployed
- Backend API běží (port 5000)
- API proxy routing
- SPA routing (FallbackResource)
- Health endpoint

### ❌ Co nefunguje / čeká se
- **Microsoft Entra ID přihlášení** - používají se dummy hodnoty
  - CLIENT_ID: `00000000-0000-0000-0000-000000000000`
  - TENANT_ID: `00000000-0000-0000-0000-000000000000`
- Čeká se na skutečné hodnoty od IT kolegy

### 🔜 TODO
- [ ] Získat skutečné EntraID credentials
- [ ] Aktualizovat .env soubory (client + server)
- [ ] Rebuild & redeploy s novými hodnotami
- [ ] Otestovat přihlášení
- [ ] Nastavit systemd service pro API (místo nohup)

---

## 📝 Poznámky

### Git repository
- **Owner:** robex08
- **Repo:** eeo2025-erdms
- **Branch:** main (default)

### Node.js verze
- Používá se přes NVM (`~/.nvm/nvm.sh`)
- Verze: 20.x

### PHP verze
- **CLI:** PHP 8.4.11 (NTS)
- **FPM:** PHP 8.4 FastCGI Process Manager
- **Status:** ✅ Aktivní (běží od 24. října 2025)
- **Zend Engine:** v4.4.11
- **OPcache:** ✅ Aktivní
- **Config:** `/etc/php/8.4/`
- **Poznámka:** Apache nepoužívá PHP modul (mod_php), ale FastCGI (php-fpm)

### MySQL/Database
- **Server IP:** 10.3.172.11
- **Port:** 3306 (standardní MySQL port)
- **Status:** ✅ Server běží (ověřeno přes phpMyAdmin localhost:3306)
- **Verze serveru:** MariaDB 11.8.3
- **Typ serveru:** MariaDB (via UNIX socket)
- **Verze protokolu:** 10
- **Připojení:** SSL se nepoužívá ⚠️
- **phpMyAdmin:** 5.2.2deb1
- **Uživatel:** phpmyadmin@localhost
- **Znakové sady:** utf8mb4 (doporučeno pro české znaky + emoji)
- **MySQL klient:** ❌ Není nainstalovaný na web serveru (akd-www-web01)
- **Firewall:** ⚠️ Blokuje vzdálené připojení z web serveru - čeká se na opravu
- **Databázové extensions PHP:** mysqli, mysqlnd 8.4.11, pdo_mysql
- **Poznámka:** Server běží lokálně, ale není dostupný z venku kvůli firewallu

### Časové razítko
- **Poslední update:** 2. prosince 2025
- **Nasazení dokončeno:** 1. prosince 2025

---

## 🆘 Quick troubleshooting

### API nereaguje
```bash
# Zjisti, jestli běží
ps aux | grep "node src/index.js"

# Zkontroluj logy
tail -f /var/log/erdms-api.log

# Restart
kill $(cat /var/run/erdms-api.pid)
cd /var/www/erdms/api/v1.0 && nohup node src/index.js > /var/log/erdms-api.log 2>&1 &
echo $! > /var/run/erdms-api.pid
```

### Frontend vrací 404
```bash
# Zkontroluj, že build existuje
ls -la /var/www/erdms/index.html

# Zkontroluj Apache
systemctl status apache2
tail -f /var/log/apache2/erdms-443-ssl-error.log
```

### Login nefunguje
```bash
# Zkontroluj .env hodnoty
cat /var/www/eeo2025/client/.env
cat /var/www/eeo2025/server/.env

# Po změně .env vždy rebuild
cd /var/www/eeo2025/client && npm run build
cp -r dist/* /var/www/erdms/
```

---

**Tento soubor slouží jako "paměť" pro GitHub Copilot - obsahuje všechny důležité informace o projektu.**
